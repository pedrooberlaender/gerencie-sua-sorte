...
const Dashboard: React.FC = () => {
  ...
  const [bankrollInicial, setBankrollInicial] = useState<string>('0,00');
  ...
  const handleBankrollChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value === '') {
      setBankrollInicial('0,00');
    } else {
      setBankrollInicial(value);
    }
  };

  useEffect(() => {
    if (bankrollInicial && !isNaN(parseFloat(bankrollInicial.replace(',', '.')))) {
      const valor = parseFloat(bankrollInicial.replace(',', '.'));
  ...
  useEffect(() => {
    if (bankrollInicial && !isNaN(parseFloat(bankrollInicial.replace(',', '.'))) && diaAtual === 1) {
      setBancaInicialPrimeiroDia(parseFloat(bankrollInicial.replace(',', '.')));
    }
  ...
  const calcularValorMaximoEntrada = () => {
    const bancaAtual = parseFloat(bankrollInicial.replace(',', '.'));
    if (isNaN(bancaAtual)) return 0;
    const porcentagem = getValorMaximoEntradaPorcentagem(tipoJogador) / 100;
    return bancaAtual * porcentagem;
  };
  ...
  const finalizarDia = () => {
    if (resultado && !isNaN(resultado)) {
      ...
      setHistoricoBancas(prev => [...prev, { 
        dia: diaAtual, 
        banca: resultado,
        bancaInicial: diaAtual === 1 ? parseFloat(bankrollInicial.replace(',', '.')) : historicoBancas[historicoBancas.length - 1]?.banca
      }]);
      
      // Atualiza para o próximo dia
      setDiaAtual(prev => prev + 1);
      setBankrollInicial(resultado.toString().replace('.', ','));
  ...
  const voltarDia = () => {
    if (diaAtual > 1) {
      ...
      if (novoHistorico.length > 0) {
        ...
        setBankrollInicial(ultimaBanca.toString().replace('.', ','));
            ...
            <div className="p-3 sm:p-4">
              ...
              <div className="text-center mt-2">
                <button
                  ...
                  className="text-gray-400 hover:text-gray-300 text-sm sm:text-base transition-duration-300 underline underline-offset-2 hover:underline-offset-4 bg-transparent border-0"
                >
                  Já sou registrado
                </button>
              </div>
      ...
      >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
              ...
              <div>
                ...
                <div className="relative">
                  <span className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 select-none">R$</span>
                  <input
                    type="text"
                    inputMode="decimal"
                    ...
                    placeholder="0,00"
                    className="input-dark w-full p-3 pl-10 rounded-lg text-white focus:outline-none text-right"
                  />
                </div>
              </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 mt-4">
              {historicoBancas.length > 0 && (
                <button
                  ...
                  className="purple-button-outline w-full sm:w-auto py-3 sm:py-2 px-4 text-sm text-white font-semibold rounded-lg hover:transform hover:scale-105 flex items-center justify-center"
                >
              ...
              <div className="file-input-wrapper w-full sm:w-auto">
                <input
                  type="file"
                  accept=".json"
                ...
                <label
                  ...
                  className="purple-button-outline w-full py-3 sm:py-2 px-4 text-sm text-white font-semibold rounded-lg hover:transform hover:scale-105 flex items-center justify-center cursor-pointer"
                >
                  <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="text-purple-300 font-medium text-lg">Jogue com responsabilidade</p>
          </div>
        </motion.div>

        {/* Footer */}
        <footer className="w-full mt-8 pb-4 px-4">
          <div className="max-w-3xl mx-auto">
            <div className="glass-card rounded-lg p-4">
              <div className="text-center text-sm text-gray-400">
                <p className="mb-2"> 2024 Gerencie Sua Sorte. Todos os direitos reservados.</p>
                ...
                <p className="mt-2 text-xs text-gray-500">
                  Este é um site de gerenciamento de banca. Não nos responsabilizamos por perdas financeiras.
                </p>
              </div>
            </div>
          </div>
